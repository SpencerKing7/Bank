...
- **Realtime transport**: Firestore is initialized with `experimentalForceLongPolling` ([src/firebase.ts](src/firebase.ts)). The default streaming transport gets buffered by some mobile-carrier and firewall proxies, which leaves a device stuck exactly one update behind until it refreshes; discrete long-polling has no open stream to buffer, so every device stays current. `useForegroundSync` additionally re-syncs when a phone wakes or reconnects.
